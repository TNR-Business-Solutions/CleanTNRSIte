...
async function getProducts(instanceId, options = {}) {
  try {
    ...
    if (error.response?.status === 401) {
      ...
      throw new Error(
        'Permission denied. Make sure the app has "Manage Stores" permissions.'
      );
    } else {
...
async function bulkUpdateProducts(instanceId, productUpdates) {
  try {
    ...
    console.log(
      `✅ Bulk update completed: ${results.filter((r) => r.success).length}/${
        results.length
      } successful`
    );
    return results;
...
async function updateInventory(instanceId, productId, variantId, quantity) {
  try {
    ...
    const result = await client.patch(
      `/stores/v1/inventory/products/${productId}`,
      updates
    );
...
async function getCollections(instanceId) {
  try {
    ...
    console.log(
      `✅ Retrieved ${response.collections?.length || 0} collections`
    );
...
async function createAdvancedFilter(instanceId, filterConfig) {
  try {
    ...
    // Price range filter
    // Wix API expects price filter using price.price field with numeric values
    if (filterConfig.priceRange) {
      filter["price.price"] = {
        $gte: Number(filterConfig.priceRange.min),
        $lte: Number(filterConfig.priceRange.max),
      };
    }
    ...
    // In stock filter
    // Wix API expects boolean values, not strings
    ...
    console.log(
      `✅ Advanced filter returned ${response.products?.length || 0} products`
    );
...
async function syncProductsToExternal(instanceId, platform, productIds = []) {
  try {
    console.log(
      `🔄 Syncing ${productIds.length || "all"} products to ${platform}`
    );
