...
module.exports = async (req, res) => {
  ...
  const pageAccessToken =
    req.method === "POST" ? req.body.pageAccessToken : req.query.token;
  ...
  if (!pageAccessToken) {
    return res.status(400).json({
      ...
      error: "Missing token",
      message:
        'Please provide a Facebook Page Access Token as "pageAccessToken" in POST body or "token" in query string',
    });
  }
  ...
  try {
    ...
    const detailsResponse = await axios
      .get("https://graph.facebook.com/v19.0/me", {
        params: {
          access_token: pageAccessToken,
          fields:
            "id,name,category,fan_count,followers_count,instagram_business_account",
        },
        timeout: 10000,
      })
      .catch((err) => {
    ...
    if (pageInfo.isUserToken || !pageInfo.category) {
      return res.status(400).json({
        ...
        help: {
          issue:
            "You copied the Long-Lived User Token instead of the Page Access Token",
          solution:
            'Please reconnect to Facebook OAuth and copy the "Page Access Token" (the one under your page name, not at the top)',
    ...
    try {
      ...
      console.log(
        "Token expires at:",
        tokenDebugInfo.expires_at
          ? new Date(tokenDebugInfo.expires_at * 1000)
          : "Never"
      );
    } catch (debugError) {
      ...
      try {
        ...
        if (igError.response) {
          ...
          if (igError.response.status === 403) {
            console.warn(
              "⚠️ Permission denied - Your token may not have Instagram permissions"
            );
            console.warn(
              "   Solution: Reconnect via OAuth with pages_manage_posts permission"
            );
          } else if (igError.response.status === 400) {
            console.warn(
              "⚠️ Invalid request - Instagram account may not be properly connected"
            );
            console.warn(
              "   Solution: Go to Facebook Page Settings → Instagram → Reconnect Account"
            );
          }
        }
      }
