...
// Development: Use localhost (HTTP for local dev, HTTPS for production)
function getRedirectUri() {
  // Always use production redirect URI if explicitly set
  if (process.env.WIX_REDIRECT_URI) {
    return process.env.WIX_REDIRECT_URI;
  }
  ...
  if (process.env.VERCEL || process.env.VERCEL_ENV || process.env.NODE_ENV === 'production') {
    ...
    return "https://www.tnrbusinesssolutions.com/api/auth/wix/callback";
  }

  // Development: Use HTTP localhost (Wix accepts HTTP for localhost)
  // IMPORTANT: You must add this exact URL to your Wix app's redirect URIs
  return "http://localhost:3000/api/auth/wix/callback";
}
