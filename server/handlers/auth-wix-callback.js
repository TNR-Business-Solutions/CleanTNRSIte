...
async function exchangeCodeForToken(code) {
  try {
    ...
    if (response.data.access_token) {
      ...
      console.log(`   Access token preview: ${response.data.access_token.substring(0, 30)}...`);
    }
...
async function getInstanceDetails(accessToken) {
  try {
    ...
    console.error('❌ Error fetching instance details:', error.response?.data || error.message);
...
module.exports = async (req, res) => {
  try {
    ...
    const isJWTToken = code && (code.startsWith('OAUTH2.') || code.startsWith('eyJ'));
    ...
    console.log(`📍 Instance ID: ${instanceId || 'Will be obtained from token'}`);
    ...
    const successUrl = new URL(stateData.returnUrl, `http://${req.headers.host}`);
    successUrl.searchParams.append('success', 'true');
    successUrl.searchParams.append('instanceId', finalInstanceId);
